...
void dtNavMesh::getWorldSizeRef(float &w, float &h) {
	float Min[3] = { 0,0,0 }, Max[3] = { 0,0,0 };

	for (int i = 0; i < m_maxTiles; i++) {
		const dtMeshTile* tile = &m_tiles[i];
		if (!tile->header)
			break;
		Min[0] = dtMin(Min[0], tile->header->bmin[0]);
		Min[2] = dtMin(Min[2], tile->header->bmin[2]);
		Max[0] = dtMax(Max[0], tile->header->bmax[0]);
		Max[2] = dtMax(Max[2], tile->header->bmax[2]);
	}
	...
	//	float worldMinZ = tile->header->bmin[0] + n->bmin[2] * cs;

	//	float worldMaxX = tile->header->bmax[0] + n->bmax[0] * cs;
	//	float worldMaxY = tile->header->bmax[0] + n->bmax[1] * cs;
	//	float worldMaxZ = tile->header->bmax[0] + n->bmax[2] * cs;
	//	// Etc...
	//	i = 0;
	//}
}
